...
public class Customer : baseObject
{
    ...
    {
        string sql = "select * from Fin_Customer where ID = " + id.ToString();
    ...
    public void updateCustomerItemBalance(int CustID, int ProdID, Decimal paidAmt)
    {
        string sql = "update Fin_CustomerItems set Balance = Balance - " + paidAmt.ToString() + " where CustomerID = " + CustID.ToString() + " and ProductID = " + ProdID.ToString();
        DataAccess.ExecuteSQLNonQuery(sql);
    }
    public DataTable getCustomerItemList(int customerID)
    {
        string sql = "select * from Fin_CustomerItems inner join Fin_Product on Fin_CustomerItems.ProductID = Fin_Product.ID where CustomerID = " + customerID.ToString();
        DataTable dt = DataAccess.ExecuteSQLQuery(sql);
        return dt;
    }
    public override DataTable getList()
    {
        string sql = "SELECT     dbo.Fin_Customer.ID, dbo.Fin_Customer.Name, dbo.Fin_Customer.CreationDate, dbo.Fin_Customer.LastModifiedDate, SUM(dbo.Fin_CustomerItems.Price) AS TotalBuyedAmount, SUM(dbo.Fin_CustomerItems.Balance) AS TotalBalance FROM  dbo.Fin_Customer FULL OUTER JOIN dbo.Fin_CustomerItems ON dbo.Fin_Customer.ID = dbo.Fin_CustomerItems.CustomerID GROUP BY dbo.Fin_Customer.ID, dbo.Fin_Customer.Name, dbo.Fin_Customer.CreationDate, dbo.Fin_Customer.LastModifiedDate";
    ...
    public override int save()
    {
        ...
        string sql = "insert into Fin_Customer(CreationDate,LastModifiedDate,OperatorID,Name) values (@Creationdate,@LastModifiedDate,@OperatorID,@Name)";
        DataAccess.ExecuteSQLNonQuery(sql, param);
        //get last id
        sql = "select max(id)as lastID from Fin_Customer";
        DataTable dt = DataAccess.ExecuteSQLQuery(sql);
        if (dt != null && dt.Rows != null && dt.Rows.Count > 0)
            return int.Parse(dt.Rows[0]["lastID"].ToString());
        return 0;
    ...
    public override int update()
    {
        ...
        string sql = "update Fin_Customer set LastModifiedDate = @LastModifiedDate,OperatorID = @OperatorID,Name = @Name where ID = @ID";
